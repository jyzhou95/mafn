...
for (year in c(2003:2018)){
  ...
  for (month in c(1:12)){
    ...
    for (i in 1:nrow(dt_pairs_curr)){
      ...
      temp1 = dt_price_curr[symbol == symb1]
      temp2 = dt_price_curr[symbol == symb2]
    ...
    #BOILERPLATE
    init_date = paste(as.character(as.Date(paste0(year, "-", month, "-01"))), "09:30:00")
    .orderqty <- 50
    init_equity = prev_portfolio_val
    .threshold <- 0.0005
    .txnfees <- 0
    .stoploss <- 1.0 # 0.003 or 0.3%
